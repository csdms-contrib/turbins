                    ...
                    if (status == IMMERSED) {
                        ...
                        for (g=0; g<ib_node->n_fluid; g++) {
                            ...
                            if ( (ii == i) && (jj == j) && (kk == k) ) { /* current immersed node is a function of itself value */
                                ...
                                if ((ib_node->fluid_coef[g]) >= LSYS_DROP_TOLERANCE) {
...
/* This function computes the total vicous dissipation of due to the fluid motion */
/* e= 2.0/Re int( S_ij S_ij dV) (over entire domain) */
void Velocity_compute_viscous_dissipation_rate_OLD(Velocity *u, Velocity *v, Velocity *w, MAC_grid *grid, Parameters *params) {
    ...
    /* It is defined as
  e = 2/Re int( S_ij S_ij dV) (over entire domain)
 */
        ...
        for (j=Js; j<Je; j++) {
            ...
            for (i=Is; i<Ie; i++) {
                ...
                if (Grid_get_c_status(grid, i, j, k) == FLUID) {  /*only include if point is fluid*/
                    ...
                    double dV = dy*dz*dx;
                    G_dissipation_rate += (S_xx*S_xx + S_yy*S_yy + S_zz*S_zz + 2.0*(S_xy*S_xy + S_xz*S_xz + S_yz*S_yz) ) * dV;
    ...
    G_dissipation_rate *= -2.0/params->Re;
    ...
    /* Store the value in all the velocity fields */
    ...
    PetscPrintf(PCW, "Velocity.c/ Dissipation rate method 2: %2.14f\n", W_dissipation_rate);
        ...
        for (j=Js; j<Je; j++) {
            for (i=Is; i<Ie; i++) {
                ...
                if (vel->component == 'u') {
                    ...
                    //data[k][j][i] = sin(2.0*PI*yq[j]/params->Ly)*sin(2.0*PI*xq[i]/params->Lx);
                    //data[k][j][i] = -cos(2.0*PI*yq[j]/params->Ly)*cos(2.0*PI*xq[i]/params->Lx);
