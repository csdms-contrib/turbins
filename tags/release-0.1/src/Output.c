...
void Output_immersed_info(Output *output, MAC_grid *grid, Parameters *params, char which_quantity) {
    ...
    /* filename */
    sprintf(filename, "%s%c_immersed_P%d.dat", output->immersed_dir, which_quantity, rank);
...
void Output_write_ascii_sed_rate(Output *output, Concentration **c, MAC_grid *grid, Parameters *params, double time) {
    ...
    if (params->rank == MASTER) {
        ...
        for (iconc=0; iconc<params->NConc; iconc++) {
            ...
            if (c[iconc]->Type  == PARTICLE) {
                /* Open the file */
                sprintf(filename, "%sS_sedim_rate_bottom_c%d.dat", output->data_dir, iconc);
...
void Output_flow_properties(Output *output, GVG_bag *data_bag) {
    ...
    if (params->sedim_rate_output) {
        ...
        Output_write_ascii_sed_rate(output, c, grid, params, time) ;
