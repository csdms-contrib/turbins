...
void Extract_wall_shear(Velocity *u, Velocity *v, Velocity *w, MAC_grid *grid, Parameters *params) {
    ...
    ierr = DAVecGetArray(grid->DA_3D, u->G_data, (void ***)&u_data_bc);PETScErrAct(ierr);
    ierr = DAVecGetArray(grid->DA_3D, v->G_data, (void ***)&v_data_bc);PETScErrAct(ierr);
    ierr = DAVecGetArray(grid->DA_3D, w->G_data, (void ***)&w_data_bc);PETScErrAct(ierr);
