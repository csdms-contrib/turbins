...
*/
    ...
    if (i_ig != NOT_FOUND) {
        ...
        PetscPrintf(PCW, "Immersed.c/ Warning. Not a single fluid node was found for the current %c-immersed node at (i,j,k)=(%d,%d,%d) \n", which_quantity, ib_node->im_index.x_index, ib_node->im_index.y_index, ib_node->im_index.z_index);
        Display_immersed_node(ib_node);
        getchar();
    } /* else */
}
...
Indices  Immersed_find_box(ImmersedNode *ib_node, PointType *p, MAC_grid *grid, char which_quantity) {
    ...
    if (found) {
        ...
        PetscPrintf(PCW, "Immersed.c/ Warning!!! A box is not found to surround the immersed node at ig(i,j,k)=(%d,%d,%d) \n", ib_node->im_index.x_index, ib_node->im_index.y_index, ib_node->im_index.z_index);
        PetscPrintf(PCW, "Immersed.c/ Now, assigning only the closest fluid node for the interpolation\n");
        ...
        if (found) {
            ...
            PetscPrintf(PCW, "Immersed.c/ Warning!!! Could not find even one fluid node neighboring current image node. Possible reason could be due to the mistakes in the definition of the solid surface. \n");
            ib_node->n_fluid = 0;
        }
    }
